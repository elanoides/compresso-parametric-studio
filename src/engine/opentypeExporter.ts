...
function segmentsToOpentypePath(segments: readonly PathSegment[]): opentype.Path {
  ...
  for (const seg of segments) {
    switch (seg.type) {
      case 'M':
        path.moveTo(Math.round(seg.x), Math.round(seg.y));
        break;
      case 'L':
        path.lineTo(Math.round(seg.x), Math.round(seg.y));
        break;
      case 'C':
        path.curveTo(
          Math.round(seg.x1),
...
export function buildFontBinary(
  ...
  const font = new opentype.Font({
    ...
    manufacturer: 'CRT Parametric Font Studio',
    designer: 'CRT Parametric Font Studio',
    description: `Generated by CRT Parametric Font Studio — ${family} ${styleName}`,
  });
